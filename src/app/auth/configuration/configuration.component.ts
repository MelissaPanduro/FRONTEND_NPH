...
export class ConfigurationComponent implements OnInit {
  ...
  getPasswordStrength(): { text: string; color: string; width: string } {
    ...
    if (!password) {
      return { text: "No especificada", color: "text-gray-400", width: "0%" }
    }

    if (password.length < 6) {
      return { text: "Débil", color: "text-red-500", width: "25%" }
    }
    ...
    if (password.length >= 8 && strength >= 3) {
      return { text: "Fuerte", color: "text-green-500", width: "100%" }
    } else if (password.length >= 6 && strength >= 2) {
      return { text: "Media", color: "text-yellow-500", width: "65%" }
    } else {
      return { text: "Débil", color: "text-red-500", width: "35%" }
    }
  }
}
